...
class WorldDatabase:
    ...
    def load(self):
        ...
        try:
            ...
            if os.path.exists(BACKUP_FILE):
                shutil.copy(BACKUP_FILE, WORLD_FILE)
                with open(WORLD_FILE, "r") as file:
                    self.data = json.load(file)
            else:
                self.save()
        ...
        self.data.setdefault("quests", {})


    def save(self):
        serialized = self.serialize(self.data)

        if os.path.exists(WORLD_FILE):
            shutil.copy(WORLD_FILE, BACKUP_FILE)
    ...
    def serialize(self, value):
        ...
        if hasattr(value, "__dict__"):
            return {key: self.serialize(item) for key, item in value.__dict__.items()}
    ...
    def get_room(self, room_id):
        return self.data["rooms"].get(room_id)


    def add_room(self, room_id, room_data):
        self.data["rooms"][room_id] = room_data
        self.save()


    def add_npc(self, npc_id, npc_data):
        self.data["npcs"][npc_id] = npc_data
        self.save()


    def get_npc(self, npc_id):
        return self.data["npcs"].get(npc_id)
